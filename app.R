...
table2 <- left_join(grades, courses)
    ...
    dashboardBody(
        tabItems(
            ...
            tabItem(tabName = "courses",
                    ...
                    fluidRow(
                        valueBoxOutput("CourseCount"),
                        valueBoxOutput("MCourseCount"),
                        valueBoxOutput("ECourseCount")
                    ...
                    fluidRow(
                        ...
                        box(
                            title = "Average Grade", height = 190,
                            textOutput("value"))
                    ),
                    
                    fluidRow(
                        box(title = "Number of Students",
                            plotOutput("plot2", height = 450)
                        ),
                        box(title = "Percent of Students who passed the Course",
...
server <- function(input, output) {
    ...
    output$plot2 <- renderPlot({
        ...
        table2 %>%
            ...
            ggplot(aes(y = students,x = as.factor(Year))) +
            geom_bar(position="dodge", stat="identity") +
            labs(x="Year", y= "Number of Students") +
    ...
    output$plot3 <- renderPlot({
        ...
        table2 %>%
            ...
            mutate(frac_passed = (passed/students)*100) %>%
            ggplot(aes(y = frac_passed,x = as.factor(Year))) +
            geom_bar(position="dodge", stat="identity") +
            scale_fill_brewer(palette="Reds") +
            theme_minimal() +
    ...
    output$CourseCount <- renderValueBox({
        valueBox(
            paste0(CourseCount), "Total Number of Courses", icon = icon("pen-nib"),
            color = "red"
        )
    })
    
    output$MCourseCount <- renderValueBox({
        valueBox(
            paste0(MCourseCount), "Number of Mandatory Courses", icon = icon("pen-nib"),
            color = "red"
        )
    })
    
    output$ECourseCount <- renderValueBox({
        valueBox(
            paste0(ECourseCount), "Number of Elective Courses", icon = icon("pen-nib"),
            color = "red"
        )
    })
